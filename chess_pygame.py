...
]  # {'from': (r,c), 'to': (r,c), 'promo': Optional['q','r','b','n']}
...
def gen_pawn_moves(board: Board, r: int, c: int, color: str) -> List[Move]:
    ...
    for dc in (-1, 1):
        ...
        if in_bounds(rr, cc):
            target = board[rr][cc]
            if target and (
                (color == "w" and is_black(target))
                or (color == "b" and is_white(target))
            ):
                if rr == last_rank:
...
def gen_knight_moves(board: Board, r: int, c: int, color: str) -> List[Move]:
    ...
    for dr, dc in deltas:
        ...
        if (
            target is None
            or (color == "w" and is_black(target))
            or (color == "b" and is_white(target))
        ):
            moves.append({"from": (r, c), "to": (rr, cc), "promo": None})
    return moves
    ...
    for dr, dc in directions:
        ...
        while in_bounds(rr, cc):
            ...
            if target is None:
                ...
                if (color == "w" and is_black(target)) or (
                    color == "b" and is_white(target)
                ):
                    moves.append({"from": (r, c), "to": (rr, cc), "promo": None})
                break
            rr += dr
...
def main():
    ...
    while True:
                ...
                if my < HEIGHT:
                    ...
                    if selected is None:
                        ...
                        if piece and is_white(piece):
                            ...
                            legal_for_selected = [
                                m for m in all_legal if m["from"] == (r, c)
                            ]
                    else:
                        # coba gerak
                        candidate = None
        ...
        draw_highlights(
            screen, selected, legal_for_selected, in_check_square, last_move
        )
